...
public class Users {
  ...
  public static boolean addUser(String username, String password) throws SQLException {
    ...
    if (password.length() < 8) {
      System.out.println("Password for username: " + username + " needs to contain atleast 8 characters!");
      return false;
    } 
    ...
    try {
      ...
      String userAddQuery = "INSERT INTO ProjTest.users" + "(username,password)" + "VALUES(" + "\""
          + username + "\"" + "," + "\"" + password + "\"" + ")";
  ...
  public static boolean deleteUser(String username) throws SQLException {
    ...
    try {
      ...
      String userDeleteQuery =
          "DELETE FROM ProjTest.users " + "WHERE username = " + "'" + username + "'";
      
      // Create query to check if username exists
      String checkUsername = "SELECT EXISTS(SELECT * FROM ProjTest.users WHERE username = " + "\"" + username + "\") AS 'check';";
  ...
  public static String printUsersTable() {
    ...
    try {
      ...
      result = statement.executeQuery("SELECT * FROM ProjTest.users");
    ...
    if (usersString == null) return "Table is Empty!"; // if no entries
    else return usersString; // if there are entries
  }
